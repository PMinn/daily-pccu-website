...
export default function LineSetting({ weatherLocations }) {
  ...
  const weatherLocationsOnClick = e => {
    let target = e.target;
  ...
  return (
    <div>
      ...
      <div class="setting-scroll">
        {
          settingsFunction.map(f => {
            return (
              <div className={"setting-li " + (view == f.id ? "inView" : "")} onClick={changeView} data-id={f.id}>
      ...
      <div class="views-scroll">
        <div class={"view " + (view == "weather" ? "inView" : "")}>
          ...
          {
            weatherLocations.map(f => {
              return (
                <div class={"location" + (f.checked ? " checked" : "")} onClick={weatherLocationsOnClick} data-name={f.location}>
                  <div class="label">
                    <div>{f.location}</div>
                    <div>{f.city}</div>
                  </div>
...
export async function getServerSideProps(context) {
  ...
  const dbRef = ref(db);
  var weatherLocations = await get(child(dbRef, 'weatherLocations')).then(snapshot => snapshot.val());
  weatherLocations = weatherLocations.map(l => {
